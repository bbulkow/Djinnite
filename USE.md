...
Djinnite uses a layered configuration system: **local project config with package fallback**.

- **`ai_config.json`** (API keys and secrets) must live in your project's `config/` directory.
- **`model_catalog.json`** and **`known_model_defaults.json`** ship with the Djinnite package and are used automatically. You only need to copy them into your project if you want to override the defaults.

This means a minimal setup only requires creating `ai_config.json` -- you do not need to copy the model catalog or defaults files.
...
```bash
# First time setup — install dependencies (add --extra dev if you plan to run tests):
uv sync                  # Runtime deps only
uv sync --extra dev      # Runtime + dev tools (pytest, pytest-cov)
...
Djinnite uses **local project config with package fallback**. For each config file, it checks the project's `config/` directory first, then falls back to the package's own `config/` directory.

**Resolution order for each config file:**
1.  **Project config dir** (CWD `config/` or parent-of-package `config/`) -- checked first.
2.  **Package config dir** (`djinnite/config/`) -- used as fallback if the file is not found in the project.

This means `model_catalog.json` and `known_model_defaults.json` are automatically picked up from the Djinnite distribution unless you place your own copies in your project's `config/` directory.

**Writes** (from `update_models`, `update_model_costs`) always go to the project config dir when it exists, otherwise to the package config dir.

#### Recommended Structure (Integrated Use -- Minimal)
```
your-project/
├── config/
│   └── ai_config.json        # Your secrets (required)
└── djinnite/                 # This module (submodule or directory)
    ├── config/
    │   ├── model_catalog.json       # Package default (used automatically)
    │   └── known_model_defaults.json # Package default (used automatically)
    └── ...
```

#### Recommended Structure (Integrated Use -- With Overrides)
```
your-project/
├── config/
│   ├── ai_config.json        # Your secrets (required)
│   ├── model_catalog.json    # Project override (takes priority)
│   └── known_model_defaults.json  # Project override (takes priority)
└── djinnite/                 # This module (submodule or directory)
    └── ...
```
